...
exports.guardControlPlanePath = guardControlPlanePath;
const ALLOWED_PREFIXES = ["system/", "user/"];
 ...
 */
function guardControlPlanePath(raw) {
    ...
    if (/[\\#?]/.test(clean))
        return { ok: false, reason: "path contains a forbidden character (\\ # ?)" };
    if (/%(2e|2f|5c)/i.test(clean))
        return { ok: false, reason: "percent-encoded dots/slashes are not allowed" };
    if (/[\s\x00-\x1f\x7f]/.test(clean))
        return { ok: false, reason: "path contains whitespace or control characters" };
    ...
    for (const seg of clean.split("/")) {
        if (seg === "." || seg === "..")
            return { ok: false, reason: "dot-segments are not allowed" };
    }
    ...
    if (resolved.origin !== base.origin)
        return { ok: false, reason: "path escapes the DreamFactory origin" };
    if (!resolved.pathname.startsWith(base.pathname))
        return { ok: false, reason: "path escapes /api/v2" };
    ...
    if (rel !== clean)
        return { ok: false, reason: "path changed under URL normalisation" };
