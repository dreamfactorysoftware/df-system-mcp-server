 ...
 */
function registerRoleTools(server, opts) {
    // Schema for a single role_service_access entry. Mirrors the DB table.
    const accessEntry = zod_1.z
        .object({
        service_id: zod_1.z.number().int().describe("The id of the service this access rule applies to."),
        component: zod_1.z
            .string()
            .default("*")
            .describe('Component path on the service, e.g. "*" for all, "_table/*" for all tables, or "_table/customers".'),
        verb_mask: zod_1.z
            .number()
            .int()
            .describe("Bitmask of allowed HTTP verbs. GET=1, POST=2, PUT=4, PATCH=8, DELETE=16. Sum them for combos (e.g. 31 = all)."),
        requestor_mask: zod_1.z
            .number()
            .int()
            .default(3)
            .describe("1 = API only, 2 = Script only, 3 = both. Almost always 3."),
        filters: zod_1.z.array(zod_1.z.record(zod_1.z.unknown())).optional().describe("Row-level filter rules (advanced)."),
        filter_op: zod_1.z.enum(["AND", "OR"]).optional(),
    })
        .describe("One row in role_service_access — a single permission grant.");
    (0, define_1.defineTool)(server, opts, "list_roles", "List all DreamFactory roles. Roles bundle a set of per-service permissions (via role_service_access). " +
        ...
        fields: zod_1.z.string().optional().describe("Comma-separated columns to return."),
        ...
        const result = await (0, dreamfactory_1.dreamFactoryFetch)("GET", "system/role", {
            auth,
            query: {
                filter,
                fields,
        ...
        return (0, dreamfactory_1.toToolResponse)("list_roles", result);
    });
    (0, define_1.defineTool)(server, opts, "get_role", "Retrieve a single role by id, INCLUDING its full role_service_access list (the permission rows). " +
        "Use this when you need to read what a role actually grants — the bare role record only carries metadata.", {
        id: zod_1.z.number().int().describe("Numeric role id."),
    }, async ({ id }, extra) => {
        const auth = (0, dreamfactory_1.getAuthForSession)(extra.sessionId);
        const result = await (0, dreamfactory_1.dreamFactoryFetch)("GET", `system/role/${id}`, {
            auth,
            query: { related: "role_service_access_by_role_id" },
        });
        return (0, dreamfactory_1.toToolResponse)("get_role", result);
    });
    (0, define_1.defineTool)(server, opts, "create_role", "Create a new DreamFactory role with optional inline service-access grants. " +
        ...
        name: zod_1.z.string().describe('Human-readable role name, e.g. "ReadOnlyReports".'),
        description: zod_1.z.string().optional(),
        is_active: zod_1.z.boolean().optional().default(true),
        role_service_access_by_role_id: zod_1.z
            .array(accessEntry)
            .optional()
        ...
        const auth = (0, dreamfactory_1.getAuthForSession)(extra.sessionId);
        const result = await (0, dreamfactory_1.dreamFactoryFetch)("POST", "system/role", {
            auth,
        ...
        return (0, dreamfactory_1.toToolResponse)("create_role", result);
    });
    (0, define_1.defineTool)(server, opts, "update_role", "Patch an existing role. To add/remove permission rows, include `role_service_access_by_role_id` " +
        ...
        patch: zod_1.z
            ...
            .describe("Partial role object. May include nested role_service_access_by_role_id to replace the access list."),
    }, async ({ id, patch }, extra) => {
        const auth = (0, dreamfactory_1.getAuthForSession)(extra.sessionId);
        const result = await (0, dreamfactory_1.dreamFactoryFetch)("PATCH", `system/role/${id}`, {
            auth,
            query: { related: "role_service_access_by_role_id", fields: "*" },
            body: patch,
        });
        return (0, dreamfactory_1.toToolResponse)("update_role", result);
    });
}
